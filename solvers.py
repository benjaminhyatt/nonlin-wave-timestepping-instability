...
class EigenvalueSolver(SolverBase):
    ...
    **kw :
    ...
    def solve_dense(self, subproblem, pars=[False], rebuild_matrices=False, left=False, normalize_left=True, **kw):
        """
        ...
        pars : array, optional
            If array with nonzero elements is supplied, applies a shift matrix
            associated with a VonNeumann analysis to M_min assuming the
            perturbation is represented under a ComplexFourier basis
            If supplied, format is: [flag, L, c, t_step, steps/stages]
            where
                flag = 1 -> assumes a multi-step method with the last argument 
                standing for number of steps, and
                flag = 2 -> assumes a multi-stage (RK) method with the last 
                argument standing for number of stages
                L -> length of 1D ComplexFourier domain
                c -> speed of propagation of background wave
                t_step -> time step size in scheme of interest
        ...
        **kw :
        ...
        if not pars[0]:
            ...
            # multi-step schemes
            if flag == 1:
                flag, L, c, t_step, steps = pars
                steps = int(steps)
                print('shifting assuming', steps, 'step BDF method')
                Nm = int(A.shape[0]/steps/2)
                k = np.array(())
                for ss in range(steps):
                    k = np.concatenate((k, np.concatenate((np.arange(Nm+1), np.arange(-Nm+1, 0)))))
                k *= 2*np.pi/L
                S = np.diag(np.exp(-1j*c*t_step*k))
                M_shift = sp.M_min.A @ sp.pre_right.A.T @ S @ sp.pre_right.A
            # multi-stage schemes
            if flag == 2:
                flag, L, c, t_step, stages = pars
                stages = int(stages)
                print('shifting assuming', stages, 'stage RK method')
                Nm = int(A.shape[0]/stages/2)
                k = np.array(())
                for ss in range(stages):
                    if ss == stages-1:
                        k = np.concatenate((k, np.concatenate((np.arange(Nm+1), np.arange(-Nm+1, 0)))))
                    else:
                        k = np.concatenate((k, np.zeros(int(A.shape[0]/stages))))
                k *= 2*np.pi/L
                S = np.diag(np.exp(-1j*c*t_step*k))
                M_shift = sp.M_min.A @ sp.pre_right.A.T @ S @ sp.pre_right.A
    ...
    def solve_sparse(self, subproblem, N, target, pars=[False], rebuild_matrices=False, left=False, normalize_left=True, raise_on_mismatch=True, v0=None, **kw):
        """
        ...
        if not pars[0]:
            ...
            if flag == 1:
                ...
                for ss in range(steps):
                    k = np.concatenate((k, np.concatenate((np.arange(Nm+1), np.arange(-Nm+1, 0)))))
                k *= 2*np.pi/L
                S = diags(np.exp(-1j*c*t_step*k))
                M_shift = sp.M_min @ sp.pre_right.T @ S @ sp.pre_right
            # multi-stage schemes
            if flag == 2:
                flag, L, c, t_step, stages = pars
                stages = int(stages)
